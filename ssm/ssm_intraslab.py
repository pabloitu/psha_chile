...
if __name__ == "__main__":
    from get_ab import read_mc, pick_windows, load_mfd_catalog, estimate_ab

    cfg = SSMConfig(n_neighbors=25)
    ...
    df_slab_deep = load_catalog(
        cat_paths.cat_slab_deep_dc,
        bbox=bbox,
        only_mainshocks=True,
        class_filter=None,
        max_events=cfg.max_events_debug,
    )
    df_intra = load_catalog(
        cat_paths.cat_intra_slab_dc,
        bbox=bbox,
        only_mainshocks=True,
        class_filter=None,
        max_events=cfg.max_events_debug,
    ...
    df_slab_deep["tect_class"] = "slab_deep"
    df_intra["tect_class"] = "intra_slab"
    df_all = pd.concat([df_slab_deep, df_intra], ignore_index=True)
    ...
    mc_sel = pick_windows(mc_df, start_index=-7)
