...
#        
    ...
    def set_testgrid(self, res, mask=None):
    ...
    def get_catalogSSM_varKernel(self, subcat ='full', N=2, power=1.5,
                                 mc_min=4.5, dist_cutoff=0., area_norm=True,
                                 sum_norm=True, nproc=16, 
                                 dmin=0.5, memclean=False):
...
#        
        ...
        seism = seism/np.sum(seism)
        faults = self.pdfx_faults[fault_subcat][mask==1]
        faults = faults/np.sum(faults)*np.sum(seism)
        
        logL = np.zeros((len(mw_disc_range), len(weight_disc)))

        Ntotal = 0
        for i, mw in enumerate(mw_disc_range):    
            self.set_subcatalog(name=(target_cat,
                                      np.round((mw[0] + mw[1])/2.0),1),
                                time=target_cat_time,
                                minmag=mw[0], maxmag=mw[1])
            
            omega = self.get_discrete_eqk_field(
                        (target_cat, np.round((mw[0] + mw[1])/2.0),1))
            Nt = np.sum(omega)        
            Ntotal += Nt
    ...
    def get_weighted_pdfx(self, s_subcat, f_subset, mask, m_disc,
        ...
        mu_s = self.pdfx_catalog_vk[s_subcat]
        mu_f = self.pdfx_faults[f_subset]
...
#        pdf_m = np.array(pdf_m)/np.sum
        self.pdfx_weighted[s_subcat] = np.sum(pdf_m, axis=0)
    ...
    def model2vtk(self, filename, fields, subcats, res_0, res_i,
        ...
        for attr in fields:
            ...
            if isinstance(fx, dict):  #Check that field is dict
                for subcat in subcats:
                    array_names.append(attr + '_' + subcat)
        ...
        assert data.shape[0]==self.grid.shape[0], 'Dimension not matching'
        ...
        pl.raster2vti(self.dir['vtk'] + filename + '.vti',
                       [raster_rpj], 
...
#    sf.get_catalogSSM_varKernel(N=2, dmin=0.)
#    sf.save_pdf('catalog_var', 'full')
#    sf.get_catalogSSM_varKernel(N=2, mc_min=5.0, dmin=0.02)
#    sf.save_pdf('catalog_var', 'full')
#    sf.get_catalogSSM_fixKernel(KernelSize=10., memclean=True)    
#    sf.save_pdf('catalog_fix', 'full')
#    
#    sf.load_distances('faults','grid')      
#    sf.get_faultSSM_fixKernel(KernelSize=10., memclean=True)    
#    sf.save_pdf('faults', 'full')
#    
...
#    sf.get_cdf_mask('pdfx_faults', 'full', 0.975)
#    
#     # Create test grid using the mask
#    sf.set_testgrid((0.1,0.1), mask=sf.mask_faults['full'])
#    
...
#    sf.load_distances('catalog','grid')
#    sf.load_distances('catalog','catalog')
#    sf.get_catalogSSM_varKernel(subcat='-1987',N=2)
...
#    Mw_max = 8.5
#    dmw = 0.1
#    mw_disc = np.arange(Mw_min, Mw_max + dmw, dmw)  
#    
#    training_cat = '-1987'
#    target_cat = '1987-'
...
#    sf.get_cdf_mask('pdfx_faults', 'full', 0.975)
#   
#    a = sf.get_weighted_pdfx('full','full', sf.mask_faults['full'], mw_disc,
#                             f=piecewise_linear,
#                             params=[0.778, 0.3])
#    
