...
import postprocessing_lib_v3 as pl
...
#        self.grid = np.genfromtxt(filename)     
    ...
    def get_cdf_mask(self, field, subcat, clvl):
        ...
        print('PDF masking complete, with %i used cells, with value higher \
              than %.5e' % (int(np.sum(mask)), val))
...
#        seism = seism/np.sum(seism)
    ...
    def get_magPDF_truncGR(self, mw_disc, a, b, bin_size=0.1):
        
        """
        yet to define
        """
        mw_disc_cum = np.append(mw_disc, mw_disc[-1]+bin_size)
        Mw_target = np.min(mw_disc)
        Mw_max = np.max(mw_disc)
        
        cdf_trunc = ( (np.exp(-b*np.log(10.) *
                                       (mw_disc_cum  - Mw_target)) -
                            np.exp(-b*np.log(10.) * (Mw_max - Mw_target))) /
                         (1 - np.exp(-b*np.log(10.) * (Mw_max - Mw_target))))
        pdf_trunc = -np.diff(cdf_trunc) / np.sum(-np.diff(cdf_trunc))
        self.magPDF['truncGR'] = {'mw':mw_disc,
                                  'mw_cum': mw_disc_cum,
                                  'pdf':pdf_trunc,
                                  'cdf': cdf_trunc[:-1],
                                  'scale' :  10**(a-b*Mw_target)}   
    ...
    def get_weighted_pdfx(self, s_subcat, f_subset, mask, m_disc,
        ...
        """
        Obtained a combined spatial discrete and cumulative pdg from a fault-
        and seismic-based SSM, by using previously calibrated parameters.
        Input:
            s_subcat (string): Seismic PDF subcatalog
            f_subset (string): Fault PDF subset
            mask (np.ndarray): Mask that points which cells are going to be
                weighted
            m_disct (np.ndarray): Discretization of Mw, upon which the weigh-
                ting function is defined
            f (function): A function to evaluate the weighting. For now
                is only defined as a piecewise linear function
            params (np.ndarray): For the defined function, defines the plateau
                values, beneath and forward of the low mw cutoff and high 
                cutoff, respectively.
        Output:
            self.pdfx_mw (dict): Contains the discrete pdf of each mw 
            self.cdfx_mw (dict): Contains the cumulative pdf of each mw
        """
        ...
        print(' Weighing spatial PDF completed')        
        return self.pdfx_mw, self.cdfx_mw
    
    def get_forecast(self, magDist, mw_disc):
        
#        print (mw_disc)
        ScalingFactor = self.magPDF[magDist]['pdf'] * \
                                    self.magPDF[magDist]['scale']
#        print(self.pdfx_mw[mw_disc[0]])
        print (ScalingFactor[0])
        sf.forecast = {j : ScalingFactor[i]*self.pdfx_mw[j] 
                        for i, j in enumerate(mw_disc)}
        self.forecast_cum = {}
        for mw in mw_disc:
            self.forecast_cum[mw] = np.sum(np.array([self.forecast[i]
                            for i in mw_disc if i >= mw]), axis=0)    
    # ============================================= #
    ...
    def model2vti(self, filename, fields, res_0, res_i, subsets={},
                  log_var=[], bounds=None, names={}, group_arrays=[],
                  resample='nearest',  crs_0='EPSG:4326', crs_f='EPSG:4326'):
        """
        Complete method to create a 2D image of the class attributes
        Input:
            fields (list): List of strings with the names of the selected
                class attributes to export in the vti.
            res_0 (tuple): Initial resolution of the model 
                (i.e. the grid resolution)
            res_i (tuple): Exporting resolution in the output CRS.
            subsets (dict): Dictionary that points for each field (str), which 
                subfields (str), i.e subcat/subset/mw, are going to be saved
                e.g. subset = {'pdfx_faults': ['-1987', 'full'],
                               'pdfx_catalog_vk': 'full'}
                If no value is passed, all subfields from the attr are saved
            log_var (list): Contains string pointing to the attributes, so
                the results are provided in log10.
            bounds (list): Crop the results onto a desired bounding box expre-
                ssed in the input CRS ( [x_min, y_min, x_max, y_max] )
            names (dict): Replaces the attributes names (keys) into new ones
                (items). e.g. names = {'pdfx_faults' : 'Faults'}
            group_arrays (list): Contain the attribute names, upon which 
                all its subfields are collapsed into single nD-variable 
                to be displayed in paraview.
            resample (string): Methods to interpolate the raster. e.g. nearest/
                linear/cubic/spline
            crs_0 (string): EPSG of the original model (usually lon/lat)
            crs_f (string): EPSG to export the results.
        
        Output:
            .vti File, with path self.dir['vtk'] + filename. 
            A text log in the same folder, which indicates the array names and 
            numbering.
        ...
        for attr in fields:
            ...
            # > Class attribute is multivariable array
                if attr in subsets:     
                #> The attribute(s) is flagged, so only a subset(s) is saved
                    if not isinstance(subsets[attr], (list, np.ndarray)):
                    ...
                    for cat in catalogs:
                        ...
                        if isinstance(data, (list, np.ndarray)):
                            ...
                            assert data[-1].shape[0]==self.grid_int.shape[0],\
                                'Array dimension missmatch with the grid'
                        else:
...
# =============================================================================
    ...
    sf.get_magPDF_truncGR(mw_disc, a, b, bin_size=0.1)       
    pdf, cdf = sf.get_weighted_pdfx('full','na',
                                     sf.mask_faults['na'],
                                     mw_disc,
                                     f=piecewise_linear,
                                     params=[0.7725, 0.1364])
    
    sf.get_forecast('truncGR', mw_disc)    
...
#    
#    plt.plot(np.sort(np.log10(sf.pdfx_weigh_ml)))
#    plt.plot(np.sort(np.log10(sf.pdfx_mw[5.9])))
#    print(max(np.abs((np.log10(sf.pdfx_weigh_ml)-
#                      np.log10(sf.pdfx_mw[5.9]))/
...
#    sf.load_pdf('catalog_fix', 'full')
    ...
    sf.model2vti('Full_model',
                ...
                crs_f=crs_f)
#    
    # im = sf.model2vti('Full_model',
    #                  ['pdfx_mw', 'cdfx_mw', 'forecast', 'forecast_cum'],
    #                  res_0, res_f,
    #                  subsets={'pdfx_mw' :[4.5,5.5,6.5,7.5,8.5],
    #                           'cdfx_mw' : [4.5,5.5,6.5,7.5,8.5],
    #                           'forecast' : [4.5,5.5,6.5,7.5,8.5],
    #                           'forecast_cum' : [4.5,5.5,6.5,7.5,8.5]},
    #                  names={},
    #                  log_var= ['pdfx_mw', 'cdfx_mw',
    #                           'forecast', 'forecast_cum'],
