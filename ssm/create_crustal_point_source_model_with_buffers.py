...
class PointSourceModelConfig:
    """
    Configuration for building the crustal point-source model from SSM.
    ...
    # magnitude scaling relationship (crustal)
    ...
    # constant depths for crustal sources
    hypo_depth_km: float = 15.0
    upper_seismogenic_depth_km: float = 0.0
    lower_seismogenic_depth_km: float = 30.0
...
def build_point_sources_from_ssm(
    ...
    for idx in range(n_cells):
        ...
        last = int(np.max(np.nonzero(rates_row > 0.0)[0]))
        rates_row = rates_row[: last + 1]
...
def check_mfd_consistency(
    ...
    *,
    ...
    for src in sources:
        ...
        # sources may carry fewer bins (trailing zeros trimmed after Mmax cap)
...
if __name__ == "__main__":
    ...
    # Capped SSM grid written by cap_ssm_mmax.py (fault-buffer Mmax handoff);
    # falls back to the uncapped grid if the capped file is absent.
    ssm_csv = Path("ssm_crustal_outputs/ssm_mfd_grid_capped.csv")
    if not ssm_csv.exists():
        ssm_csv = Path("ssm_crustal_outputs/ssm_mfd_grid.csv")
        print(f"[main] capped grid not found, using {ssm_csv}")

    xml_out = Path("ssm_crustal_outputs/ssm_crustal_point_sources.xml")
    depth_png = Path("ssm_crustal_outputs/ssm_crustal_point_depths.png")
