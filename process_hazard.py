...
model = hazard.hazardResults('test', './psha_chile/test_model', 17)
model.parse_db(imtl)
...
model.plot_pointcurves('PGA', poi, ax=axes,
                        plot_args={'mean_c': 'red', 'mean_s': '-', 'stats_lw': 2,
                                   ...
                                   'poes': False}, yrs=1)
# plt.show()
model.model2vti('test.vti', 'hmaps_stats', ['PGA'], levels=[0.0021030],
                res=(0.01, 0.01), res_method='nearest', crs_f='EPSG:4326',
                crop='./shp/analysis_regions/gran_valparaiso.shp')