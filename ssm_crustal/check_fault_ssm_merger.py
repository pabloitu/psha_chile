...
@dataclass
class CheckConfig:
    ssm_pre_csv: Path = C_.SSM_GRID
    ssm_post_csv: Path = C_.SSM_GRID_CAPPED
    union_geojson: Path = C_.BUFFERS_UNION
    ...
    catalog_csvs: tuple = ()
    bbox: tuple = C_.BBOX
    cap_mag: float = C_.CAP_MAG
    completeness_year: int = 1950
    audit_min_mag: float = 5.5
    out_dir: Path = C_.OUT / "merger_checks"
