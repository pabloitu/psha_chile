...
from the upp_sd/low_sd fields. Branches:
  ...
  mmax  observed max_mag + 0.2 (0.5) | WC1994 geometric (0.5)
  mfd   truncated GR (0.5) | tapered GR (0.5)

Moment is balanced against the discrete rates with M0 evaluated at bin
lower edges, matching the convention of the reference crustalfaults.xml.
...
class Config:
    ...
    # 'shapefile' takes upp_sd/low_sd per fault; 'zone' applies the uniform
    # depths below, the convention of the original crustalfaults.xml
...
def bin_moment(m, cfg):
    return 10.0 ** (cfg.mw_c1 * (m - cfg.bin_width / 2) + cfg.mw_c2)
...
def get_mmax(flt, mode, msr, cfg):
    if mode == "mobs":
        ...
        W = (flt["lsd"] - flt["usd"]) / math.sin(math.radians(flt["dip"]))
        mmax = msr.get_median_mag(flt["L"] * W, flt["rake"])
...
def balanced_mfd(mdot, b, m_max, kind, cfg):
    ...
    m0 = sum(r * bin_moment(m, cfg) for m, r in zip(mags, rates))
    k = mdot / m0
...
def write_reference(shp_path, out_path="crustal_faults_reference.xml", cfg=None):
    """Reference model with the conventions that reproduce crustalfaults.xml.

    Zone depths (0-20 km), Mmax from the WC1994 median on the zone-depth
    rupture area (no +0.2), b from the shapefile (1.0 fallback), Mmin 6.05,
    full coupling, moment balanced on the discrete rates with M0 at bin
    lower edges. See the reconciliation notes for the verification against
    the original per-bin rates.
    """
    cfg = cfg or Config(depth_mode="zone")
    faults = read_faults(shp_path, cfg)
    srcs = build_sources(faults, 1.0, "mgeo", "tgr", cfg)
    write_source_model(
        dest=str(Path(out_path).resolve()),
        sources_or_groups=srcs,
        name="Crustal faults reference (crustalfaults.xml conventions)",
        investigation_time=cfg.investigation_time,
    )
    target = sum(moment_rate(f, 1.0, cfg) for f in faults)
    print(f"reference: {len(srcs)} sources, "
          f"moment ratio={total_moment(srcs, cfg) / target:.6f}")
    return srcs


if __name__ == "__main__":
    shp = "../data/active_faults/crustal_faults_chile_updated.shp"
    write_reference(shp)
    main(shp_path=shp, out_dir=".")