...
def get_sam_basemodel(simplify=True):
    ...
    path = 'psha_chile/subduction.xml'
    ...
    sf_xml = []
    cf_xml = []
    for i, source in enumerate(sm[0]):
    ...
    for source in nodes_cf:
        ...
        nodes_cf_chile.append(subduction_polygon)
    ...
    for n, i in enumerate(nodes_cf):
        coords = i.polygon.coords
        poly = Polygon(coords)

        polys.append(poly)
        a_vals.append(i.mfd.a_val)  # store attribute

    # build one GeoDataFrame with all polygons
    ...
    # write one shapefile with all polygons
    gdf.to_file("polygons.shp")

    return nodes_cf, nodes_cf_chile, cf_xml, sm, Model_original
