...
\author{ Pablo Iturrieta\inst{1}, Jorge Jara\inst{1}, Graeme Weatherill\inst{1}, José Cembrano\inst{2}, Fabrice Cotton\inst{1}, et al.}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.65\textwidth}
    ...
    \only<2-3>{
			...
			{\vspace{0.2cm}\begin{center} \includegraphics[width=0.4\linewidth]{figures/nz_seism}\end{center}}
  ...
  \begin{column}{0.35\textwidth}
      \vspace{0.2cm}
    \RaggedLeft
    \only<2>{\includegraphics[valign=t,right,max width=0.5\linewidth]{figures/axis}}
    \only<3>{\includegraphics[valign=t,right,max width=0.5\linewidth]{figures/axis2}\hspace{0.1cm}}
  \end{column}
\end{columns}
}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.65\textwidth}
    \RaggedRight
			{\vspace{0.2cm}\begin{center} \includegraphics[width=0.8\linewidth]{figures/curva_ejemplo}\end{center}}
  \end{column}
...
\end{itemize}
	...
	\end{figure}
		...
		\begin{figure}
		\vspace*{-1.9cm}
		\includegraphics[width=0.8\textwidth]{./figures/gridded_forecast}
		\caption*{\scriptsize Gridded Forecast para sismicidad cortical del Modelo Nacional de Amenaza Sismica, Nueva Zelanda. Tasa de densidad $\mu$ para terremotos $M<5.0$, normalizada por el total anual $N$ (Iturrieta et al., 2024)}
	...
	\end{figure}
		...
		\includegraphics[width=0.9\textwidth]{./figures/catalog_forecast}
						\caption*{\scriptsize Una realización de un modelo de catálogos estocásticos de sismicidad cortical (ETAS) para Nueva Zelanda (Iturrieta et al., 2024)}
...
%						\caption*{\scriptsize Forecast para subducción, Modelo de Amenaza Sismica Europa ESHM2020 (Danciu et al., 2024)}
%
%	\end{figure}
		}
				\only<7>{
...
%						\caption*{\scriptsize Esquema de un Modelo de Movimiento de Suelo}
%
%	\end{figure}
		}
	\only<8-9>{
...
\textbf{Interfaz Subducción:}\\
	...
	\only<1>{
Un segmento:\\
	\begin{alertblock}{Balance de Momento Geodético-Sísmico}
\begin{equation*}
	\dot{M}^{\mathrm{geodetic}}=\cfrac{1}{\chi}\,\dot{M}^{\mathrm{seismic}}
\end{equation*}
\end{alertblock}

	\vspace{-0.3cm}
\begin{gather*}
	\dot{M}^{\mathrm{geodetic}}
=
\mu\,A\,v\\
\dot{M}^{\mathrm{seismic}}
=
\int_{m_{\min}}^{\infty}
\lambda_{\mathrm{MFD}}(a,b,m)\,M_0(m)\,\mathrm{d}m
\end{gather*}

{\footnotesize
	\(\lambda(a,b,m)\): Seismicity rate for $m$ \\
...
\begin{alertblock}{Gutenberg-Richter}
\begin{align*}
\lambda_{\mathrm{MFD}}(m) = \cfrac{\mathrm{d}}{\mathrm{d}t}N(M \ge m, t) = 10^{a - b\,m}
\end{align*}
...
\textbf{Truncated GR}
\begin{align*}
	\lambda_{\mathrm{MFD}}^{\mathrm{trGR}}(m)
=
\begin{cases}
10^{a - b\,m} - 10^{a - b\,m_{\max}}, & {m \le m_{\max},}\\[0.5ex]
0, & m > m_{\max}.
\end{cases}
	\end{align*}\\
	...
	\textbf{Tapered GR}
\begin{align*}
\lambda_{\mathrm{MFD}}^{\mathrm{tapGR}}(m)
=
10^{a - b\,m}\,
\exp\!\left[-10^{1.5\,(m - m_c)}\right],
	\end{align*}\\
	\vspace{0.2cm}
{\footnotesize
	\(m_{\mathrm{max}}\): Magnitud máxima. \\
	\(m_c\): Magnitud de esquina.}
	...
	\begin{alertblock}{Déficit de momento sísmico}
\begin{align*}
\dot{M}^{\mathrm{def}}_i
&=
\chi \,\dot{M}_{i}^{\mathrm{geodetic}}
-
...
\(\dot{M}^{\mathrm{cat}}_i\): tasa de momento sísmico liberado en el catálogo \\
	...
	\only<2>{			\begin{figure}
		\vspace*{1.cm}
		\includegraphics[width=0.95\textwidth]{./figures/mfd}
		\end{figure}}
	\only<3>{
		\begin{figure}
		\vspace*{-1.cm}
...
\begin{itemize}
  ...
  \item El \emph{slip rate} es uno parámetros clave: por
        ...
        mejor las fallas conocidas (incluyendo nuevos mapeos, p.ej.\ Marga--Marga) y entender
        la partición sísmica/asísmica.
...
\begin{itemize}
  ...
  \item Existe espacio para explorar nuevos desarrollos, como generar conjuntos de rupturas probabilísticas (California; Nueva Zelanda) o incluir catálogos sintéticos con modelos físicos.
...
%
%		\begin{itemize}
%			\item<1-> Caracterización de sitio ($V_{\mathrm{S}30} \, \mathrm{m}/\mathrm{s}$ o factores de amplificación)
%			\item<2-> Aceleraciones pseudo-espectrales
%			\item<3-> Espectro Uniforme de Amenaza (UHS)
%			\item<6-> Análisis Modal
%			\item<8-> Pushover ($S_a - S_d$)
%			\item<9-> Análisis Dinámico Incremental
%\only<6>{
%\[
%\mathbf{u} =
%\begin{bmatrix}
...
%		\begin{figure}
%	\vspace*{-1.3cm}
%	\includegraphics[width=0.78\textwidth]{./figures/fig1_vs30}
%		\caption*{Regional model of $V_{\mathrm{S}30} \, \mathrm{m}/\mathrm{s}$), derived from topographic data (Heath et al., 2020).}
%	\end{figure}
%		}
%		\only<2>{
%		\begin{figure}
%	\vspace*{-1.3cm}
%	\includegraphics[width=0.78\textwidth]{./figures/sa_02_map}
%		\caption*{Mapa de amenaza sísmica para acelerationes pseudo-espectrales $SA$ con $T=0.2$.}
%	\end{figure}
%		}
%	\only<3>{
%		\begin{figure}
%	\vspace*{-1.7cm}
%	\includegraphics[width=0.6\textwidth]{./figures/Iquique2}
%		\includegraphics[width=0.6\textwidth]{./figures/Iquique_sa}
%
%		\caption*{Curva de amenaza para $PGA$ (arriba) y $SA(0.2)$ (abajo).}
%	\end{figure}
%		}
%				\only<4>{
%		\begin{figure}
%	\vspace*{-0.3cm}
%	\includegraphics[width=0.87\textwidth]{./figures/Iquique_uhs}
%		\caption*{Espectro Uniforme de Amenaza para Iquique (Norte)}
%	\end{figure}
%		}
%							\only<5>{
%		\begin{figure}
%	\vspace*{-0.3cm}
%	\includegraphics[width=0.87\textwidth]{./figures/Santiago_uhs}
%		\caption*{Espectro Uniforme de Amenaza para Santiago (Centro)}
%	\end{figure}
%		}
%	\only<6>{
%		\begin{figure}
%	\vspace*{0.2cm}
%	\includegraphics[width=0.6\textwidth]{./figures/estructura}
%		\caption*{Estructura}
%	\end{figure}
%		}
%	\only<7>{
%		\begin{figure}
%	\vspace*{0.2cm}
%		\includegraphics[width=0.45\textwidth]{./figures/modo-1}
%	\includegraphics[width=0.45\textwidth]{./figures/modo-2}
%
%		\caption*{Estructura y modos}
%	\end{figure}
%		}
%			\only<8>{
%		\begin{figure}
%	\vspace*{0.2cm}
%		\includegraphics[width=0.8\textwidth]{./figures/Iquique_ADRS_pushover}
%		\caption*{ADRS}
%	\end{figure}
%		}
%				\only<9>{
%		\begin{figure}
%	\vspace*{0.2cm}
%		\includegraphics[width=0.8\textwidth]{./figures/iquique_resampled_dt0p1s}
%
%		\caption*{Registro Iquique}
%	\end{figure}
%		}
%		\end{columns}
%
...
\begin{frame}[t]
	\frametitle{Muchas Gracias!}
				\begin{figure}
			\vspace*{0.3cm}
			\includegraphics[width=0.55\textwidth]{./figures/hazardmap_faults}
			\end{figure}
\end{frame}
