# s02_build_sm.py  (INTRASLAB)
# Source model (SM): turn the intraslab SSM grid (s01) into OpenQuake
# PointSources and write the NRML. Depths come from the slab geometry
# (nearest slab node + offset), as in the original builder.
#
# Differences vs the previous version:
#   - reads paths/params from ssm_config
#   - min_mag is the first bin CENTRE (edges[0] + dM/2); it was edges[0],
#     which shifted every source half a bin low
#   - trailing zero bins are trimmed per cell, so a cell whose class Mmax is
#     below the global bin range stops at its own Mmax
...
class PointSourceModelConfig:
    ...
    rupture_mesh_spacing: float = C.RUPTURE_MESH_SPACING  # km
...
def build_point_sources_from_ssm(
    ...
    # EvenlyDiscretizedMFD.min_mag is the first bin CENTRE, and the grid
    # columns rate_M{lo}_{hi} are bin EDGES -> offset by half a bin.
    min_mag = float(mag_edges[0]) + dM / 2.0
    ...
    for idx in range(n_cells):
        ...
        # trim trailing zero bins: a cell whose class Mmax is below the
        # global bin range stops at its own Mmax
...
def check_mfd_consistency(
    ...
    *,
    ...
    for src in sources:
        ...
        # sources may carry fewer bins (trailing zeros trimmed above)
...
# ----------------------------------------------------------------------
def main():
    # 1) read the intraslab SSM grid written by s01 (per-class superposition)
    ssm_csv = C.SSM_GRID

    # 2) slab geometry: each cell's hypocentre is the nearest slab node depth
    #    plus cfg.depth_offset_km; cells with no slab within the search radius
    #    are skipped and reported
    from cat_no_mech_handler import paths as cat_paths

    # 3) build one PointSource per non-empty cell, check the MFDs against the
    #    grid, and write the NRML consumed by the logic tree
    create_point_source_model(
        ssm_mfd_csv=ssm_csv,
        slab_depth_csv=cat_paths.slab_depth,
        xml_out=C.SM_XML,
