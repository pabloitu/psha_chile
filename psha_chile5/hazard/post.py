# Contributions of the model parts (CONTRIB, exact in rate space for
# independent families) into _contrib/<split>/ and _contrib/summary.csv, then
# tables and figures of the full model (JOB) into <JOB>/post/:
...
# settings
JOB = "disagg"          # the full model: per-city curves, branches, axes
...
IF_, IS_, CR_ = ("c_if_ref", None, "Interface"), ("c_is_ref", None, "Intraslab"), ("c_cr_ref", None, "Crustal")
CONTRIB = [
    # reference: sum of the three family runs, full GMM tree per region
    {"name": "full_ref", "total": None, "parts": [IF_, IS_, CR_]},
    # in-slab classes, summed reference and class-cut smoothing
    {"name": "full_classes", "total": None,
     "parts": [IF_, ("c_is_intra_slab", None, "intra_slab"), ("c_is_slab_deep", None, "slab_deep"),
               ("c_is_deep_nest", None, "deep_nest"), CR_]},
    {"name": "full_cm_classes", "total": None,
     "parts": [IF_, ("c_is_cm_intra_slab", None, "intra_slab"), ("c_is_cm_slab_deep", None, "slab_deep"),
               ("c_is_cm_deep_nest", None, "deep_nest"), CR_]},
    # one region on a single GMM, the others on their tree
    *[{"name": f"gmm_slab_{k}", "total": None, "parts": [IF_, (f"c_is_gmm_{k}", None, "Intraslab"), CR_]}
      for k in ("ag", "pk", "mv")],
    *[{"name": f"gmm_inter_{k}", "total": None, "parts": [(f"c_if_gmm_{k}", None, "Interface"), IS_, CR_]}
      for k in ("ag", "pk", "ku")],
    *[{"name": f"gmm_crust_{k}", "total": None, "parts": [IF_, IS_, (f"c_cr_gmm_{k}", None, "Crustal")]}
      for k in ("ask", "bssa", "cb", "cy")],
    # truncation level and the full interface tree
    *[{"name": f"full_{k}", "total": None,
       "parts": [(f"c_if_{k}", None, "Interface"), (f"c_is_{k}", None, "Intraslab"), (f"c_cr_{k}", None, "Crustal")]}
      for k in ("tr20", "tr25", "tr40")],
    {"name": "full_iftree", "total": None, "parts": [("c_if_tree", None, "Interface"), IS_, CR_]},
]
