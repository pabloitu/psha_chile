...
def load(mod, over=None):
    ...
    for k, v in over.items():
        setattr(c, k, v)
    if getattr(c, "CAT_VARIANT", None):
        c.CAT = Path(c.CAT).parent / c.CAT_VARIANT / Path(c.CAT).name
    c.TAG = tag(over)
...
def snapshot(c, step, extra=None, value=True):
    """Record the settings and the finished step (its hash, see run.py) in OUT/config.json."""
    c.OUT.mkdir(parents=True, exist_ok=True)
    p = c.OUT / "config.json"
    d = json.loads(p.read_text()) if p.exists() else {}
    if d.get("config") != as_dict(c):
        d = {"steps": {k: v for k, v in d.get("steps", {}).items() if v is not True}}
    d["config"] = as_dict(c)
    d.setdefault("steps", {})[step] = value
    if extra:
