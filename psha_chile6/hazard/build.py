...
# self-contained OpenQuake job per entry: outputs/hazard/<site>/<NAME>/{job.ini,
# source_lt.xml, gmm_lt.xml, sites.csv, src/, build.json}. A job may carry "gmm": {trt: [...]}, which
...
def job(hd, desc, site=None):
    ...
    return f"""[general]
description = {desc}
calculation_mode = {"disaggregation" if dg else "classical"}
...
complex_fault_mesh_spacing = {ca.get("MESH", hc.MESH)}
...
""" + (f"""
[disaggregation]
mag_bin_width = {dg.get("mag_bin", 0.25)}
distance_bin_width = {dg.get("dist_bin", 20.0)}
coordinate_bin_width = {dg.get("coord_bin", 1.0)}
num_epsilon_bins = {dg.get("n_eps", 8)}
disagg_outputs = {" ".join(dg.get("outputs", ["Mag_Dist_Eps", "TRT_Mag_Dist"]))}
""" if dg else "")
...
def main():
    ...
    for n in names:
        ...
        lt.GMM_JOB = {**lt.GMM_FULL, **job.pop("gmm", {})}
