...
def field(elon, elat, w, h, glon, glat, power, dmax, kind="power"):
    """
    Smoothed rate on grid cells: each event spreads its weight with
    K = 1 / (r^2 + h^2)^power (kind "power") or exp(-r^2 / 2 h^2) ("gauss"),
    normalized over the cells within dmax. Cells not in the grid receive
    nothing, so masking cells before this call renormalizes the rate onto
    ...
    for i in range(len(elon)):
        ...
        if not m.any():
            continue
        k = np.exp(-r[m] ** 2 / (2 * h[i] ** 2)) if kind == "gauss" else 1.0 / (r[m] ** 2 + h[i] ** 2) ** power
        if k.sum() <= 0:
            continue
        out[m] += k * (w[i] / k.sum())
    return out
...
def slab_node(glon, glat, path):
    """Value of the nearest node of a Slab2 xyz grid to each cell and the distance to it (km)."""
    from scipy.spatial import cKDTree
    s = pd.read_csv(path, header=None, names=["lon", "lat", "val"]).dropna()
    ...
    return s["val"].to_numpy()[i], 2 * 6371.0 * np.arcsin(np.clip(d / 2, 0, 1))


def slab_top(glon, glat, path):
    """Depth (km, positive down) of the nearest Slab2 node to each cell and the distance to it (km)."""
    v, d = slab_node(glon, glat, path)
    return -v, d
