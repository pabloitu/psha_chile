...
model_with = hazard.hazardResults('Faults', './model_faults', 64)
model_with.parse_db(imtl)
model_with.get_stats('hcurves', 'PGA')

model_without = hazard.hazardResults('No_Faults', './model_nofaults', 1633)
...
for city, coords in cities.items():
    ax = model_with.plot_pointcurves('PGA', point=coords, title=city,
    ...
    extra_handles = [Patch(facecolor="0.8", edgecolor="none", alpha=0.5, label="95% envelope"),
                    Line2D([0], [0],color="black", lw=0.5,label="Mean"),]
