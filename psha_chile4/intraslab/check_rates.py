...
def main():
    ...
    for k in c.CLASSES:
        ...
        g[k] = (df, rb, e[:-1], df["slab_km"].to_numpy() + c.DEPTH_OFFSET)
    ...
    for ax in axs[-1]:
        ax.set_xlabel("M")
    for ax in axs[:, 0]:
        ax.set_ylabwel(f"N(>=M)/yr within {R_KM:g} km")
