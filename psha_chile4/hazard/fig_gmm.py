...
# Each split changes one GMM (AG20 elsewhere); "all" is the AG20 reference.
...
# settings
SUMMARY = paths.OUT / "hazard" / "_contrib" / "summary.csv"
OUT = paths.OUT / "hazard" / "_contrib" / "figures"
REF = "all"
SPLITS = {"all": ("AG20 / AG20", "0.3"),
          "gmm_slab_pk": ("slab Parker", "#dd8452"),
          "gmm_slab_mv": ("slab Montalva", "#c44e52"),
          "gmm_inter_pk": ("interface Parker", "#4c72b0"),
          "gmm_inter_ku": ("interface Kuehn", "#8172b3")}
SLAB = ["all", "gmm_slab_pk", "gmm_slab_mv"]
...
def table():
    ...
    if miss:
        raise SystemExit(f"{SUMMARY} has no split {miss}; run hazard/run_all.sh with the step 3 jobs")
...
def fig_pga(t, path):
    ...
    for ax, rp in zip(np.atleast_1d(axs), rps):
        ...
        ax.set_title(f"PGA {rp} yr: change vs AG20 everywhere (%)", fontsize=10)
