# Decluster each crustal class separately (region box and historical cutoff first).
...
def load(c):
    ...
    for k, box in c.CLASSES.items():
        if box is not None:
            lo, hi, la, ha = box
            m = (df["class"] == k).to_numpy()
            keep &= ~m | (df["longitude"].between(lo, hi) & df["latitude"].between(la, ha)).to_numpy()
